...
class GeneratedDataset(Dataset):
    def __init__(self,
        ...
        self._load_txt_files_modified(prompt_dir, src_img_dir, src_img_id)
    ...
    def __getitem__(self, index):
        ...
        prompt_id = index % 20
        gen_img_id = src_img_path.split("/")[-1].split(".")[0]
        gen_img_paths = [os.path.join(self.eval_folder, f'{gen_img_id}_{prompt_id}.jpg')]  # {IMAGE_ID}_{PROMPT_ID}.jpg

        src_img_tensor = self._img_path_to_nchw(src_img_path)  # (1,C,H,W)# 1,3,1024,1024
        gen_img_tensors = [self._img_path_to_nchw(x) for x in gen_img_paths]  # (RB,C,H,W), R=repeats, B=batch_size
        gen_img_tensors = torch.stack(gen_img_tensors, dim=0)   # 32, 3, 1024, 1024
        ...
        return {
            ...
            'gen_img_path':gen_img_paths[0],
            'image_id':gen_img_id
        }

    def _load_txt_files_modified(self, prompt_dir, src_img_dir, src_img_id):  # 提取源文件路径
        prompts_file = prompt_dir   # '/mnt/bn/motor-cv-yzh/CelebBasis/evaluation/src/prompts.txt'
        src_image_dir = src_img_dir # "/mnt/bn/motor-cv-yzh/CelebBasis/evaluation/src/200"
        in_image_ids_file = src_img_id   # '/mnt/bn/motor-cv-yzh/CelebBasis/evaluation/src/image_id.txt'

        num_prompts = 20
        num_images = 200
        
        # 所有prompts, 20 * 200
        with open(os.path.join(prompt_dir), 'r') as f:
            prompt = f.read().splitlines() # length：15
            prompt = [x[:-1] for x in prompt]
            prompts = [x.replace(" sks", "") + '.' for _ in range(num_images) for x in prompt]
        
        # photomaker
        # if input_prompt.endswith("person \n"):
        #     input_prompt = input_prompt.replace("person \n", "person img.")
        # else:
        #     input_prompt = input_prompt.replace("person", "person img ").replace(" \n", ".")
        
        # 源图路径, 20 * 200
        with open(src_img_id, 'r') as f:
            img_id = f.read().splitlines()
            src_img_paths = [os.path.join(src_image_dir, x) for x in img_id for _ in range(num_prompts)]

        # 随机种子, 32个
        # seed = list(range(42, 74))
        seed = [42] # Fixed

        self.prompts = prompts
        self.src_img_paths = src_img_paths
        self.img_id = [x.split(".")[0] for x in img_id]
        self.seed = seed
        print('[GeneratedDataset] txt files loaded.')

        with open(self.save_dir, "w", encoding='utf-8', newline='') as csvfile:
            writer = csv.writer(csvfile)
            writer.writerow(["image_id", "prompt", "sim_img", "sim_text", "id_cos_sim", "id_mse_dist", "id_l2_dist"])

    def _img_path_to_nchw(self, img_path: str):
        image = Image.open(img_path).convert('RGB')
        # need when eval instantid
        if (image.size[1] != 512):
...
class IDCLIPScoreCalculator(ABC):
    ...
    def start_calc(self):
        ...
        for data in self.data_loader:   
            src = data['src_img_tensor'].squeeze(dim=0)
            gen = data['gen_img_tensors'].squeeze(dim=0)
            src_img_path = data['src_img_path'][0]
            gen_img_path = data['gen_img_path'][0]

            if self.device.type != 'cpu':
                src = src.cuda()    # 1,3,1024,1024
                gen = gen.cuda()    # 32,3,1024,1024
            ...
            sim_img, sim_text, id_result_dict = self.id_clip_evaluator.evaluate(
                gen.repeat(1, 1, 1, 1),
                src.repeat(1, 1, 1, 1),
                prompt, src_img_path, gen_img_path)
            ...
            with open(self.save_dir, "a+", encoding='utf-8', newline='') as csvfile:
                writer = csv.writer(csvfile)
                writer.writerow([image_id, prompt, float(sim_img), float(sim_text), float(id_cos_sim), id_mse_dist, id_l2_dist])
